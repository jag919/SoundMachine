...
void setup()
{
	...
	// attachInterrupt(digitalPinToInterrupt(2), playOrPause, RISING);	//pin2 -> INT0, and the Touch Sensor 
	// attachInterrupt(digitalPinToInterrupt(3), changeTrack, RISING);	//pin3 -> INT0, and the Touch Sensor 

	sendCommand(CMD_SEL_DEV, DEV_TF);
	delay(200);
	sendCommand(CMD_SET_VOLUME, 20);
	delay(200);
	sendCommand(CMD_SINGLE_CYCLE_PLAY, 1);
}
...
void loop()
{
	...
	if (debounceButton(2, btn1) == 1 && btn1 == 0)
	{
		playOrPause();
		btn1 = 1;
	}
	else if (debounceButton(2, btn1) == 0 && btn1 == 1)
	{
		btn1 = 0;
	}

	if (debounceButton(3, btn2) == 1 && btn2 == 0)
	{
		changeTrack();
		btn2 = 1;
	}
	else if (debounceButton(3, btn2) == 0 && btn2 == 1)
	{
		btn2 = 0;
	}
...
void changeTrack()
{
	...
	Serial.print(track);
	sendCommand(CMD_SINGLE_CYCLE_PLAY, track + 1);
	interrupts();
}
